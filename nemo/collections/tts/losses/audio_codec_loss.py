...
class DiscriminatorHingedLossWithLeCam(Loss):
    def __init__(self, decay=0.99, lambda_rlc=0.3):
        ...
        As implemented as in https://arxiv.org/pdf/2104.03310
...
class DiscriminatorSquaredLossWithLeCam(Loss):
    def __init__(self, decay=0.99, lambda_rlc=0.3):
        """
        Discriminator loss = Squared loss + LeCam regularization.
        As implemented in https://arxiv.org/pdf/2104.03310
    ...
    def forward(self, disc_scores_real, disc_scores_gen):
        ...
        for d_real, d_gen in zip(disc_scores_real, disc_scores_gen):
            # Squared loss (LSGAN-style)
            loss_real = torch.mean((1.0 - d_real) ** 2)
            loss_gen = torch.mean(d_gen ** 2)
            squared_loss = (loss_real + loss_gen) / 2
            total_loss += squared_loss
...
class MMDEmbeddingLoss(Loss):
    """
    Maximum mean discrepancy (MMD) loss in embedding
    Inspired by: https://arxiv.org/abs/2406.02315
    """
...
class MMDTimeLoss(Loss):
    """
    Maximum mean discrepancy (MMD) loss in time domain
    Inspired by: https://arxiv.org/abs/2406.02315
    """
