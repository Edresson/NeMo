...
class LhotseAudioQuestionAnswerDataset(torch.utils.data.Dataset):
    ...
    def __getitem__duplex_(self, cuts) -> dict[str, torch.Tensor | list[str] | dict]:
        ...
        for i in range(len(num_turns)):
            ...
            for j in range(num_turns[i] // 2):
                ...
                # if first turn
                if j == 0:
                    audio_start_step = int(text_start_time[i][j]*self.codec_sample_rate) # multiply by sampling rate to convert seconds in frames
                    audio_end_step = int(text_end_time[i][j]*self.codec_sample_rate)# multiply by sampling rate to convert seconds in frames
    ...
    def __getitem__duplex_overlap_(self, cuts) -> dict[str, torch.Tensor | list[str] | dict]:
        ...
        for id, cut in enumerate(cuts):
            ...
            def get_answer_audio_first_turn_from_segments(segments, answer_audios, id):
                ...
                audio_start_step = int(validate_time(segment['start'])*self.codec_sample_rate) # multiply by sampling rate to convert seconds in frames
                audio_end_step = int(validate_time(segment['end'])*self.codec_sample_rate)# multiply by sampling rate to convert seconds in frames
            ...
            answer_audio_first_turn, answer_audio_first_turn_len = get_answer_audio_first_turn_from_segments(cut.agent_segments, answer_audios, id)
            answer_audios_first_turn.append(answer_audio_first_turn)
    ...
    def __getitem__tts_(self, cuts) -> dict[str, torch.Tensor | list[str] | dict]:
        ...
        for i, cut in enumerate(cuts):
            ...
            text_end_step = int(answer_audio_len / self.codec_model_downsampling_factor / self.decoder_reduction_factor) - (random_silence_tokens_right - 2)
            ...
            if cut.supervisions[1].speaker == "agent":
                ...
                cur_source_text[max(text_end_step-1, 0)] = self.text_processor.eos_id # -1 to make sure that both eos
                cur_source_text[min(text_end_step+1, cur_source_text.size(0)-1)] = self.text_processor.bos_id # +1 to make sure that both eos bos are in the tensor if we use both channels
